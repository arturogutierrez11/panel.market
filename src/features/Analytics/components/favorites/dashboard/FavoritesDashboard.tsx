...
import { useMarketplaceFavorites } from '../items/hooks/useMarketplaceFavorites';
import { useMarketplaceFavoriteActions } from '../items/hooks/useMarketplaceFavoriteActions';
...
import { FavoritesTable } from '../items/components/FavoritesTable';
...
export default function FavoritesDashboard() {
  ...
  const { data: fullOverview, loading: loadingOverview } =
    useMarketplaceFullOverview(selectedId);

  const {
    data: favorites,
    loading: loadingFavorites,
    reload: reloadFavorites,
  } = useMarketplaceFavorites(selectedId);

  const { removeFavorite } = useMarketplaceFavoriteActions();
  ...
  const handleRemove = async (productId: string) => {
    ...
    await removeFavorite(selectedId, productId);
  ...
  >
    ...
    <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
      ...
      <div className="space-y-2">
        ...
        <p className="text-sm text-zinc-400">
          Gestioná productos, métricas y rendimiento de esta carpeta.
        </p>
      </div>
    ...
    <div className="transition-all duration-300">
      {loadingOverview ? (
        ...
        <MarketplaceOverviewCards fullOverview={fullOverview} />
      ) : null}
    </div>
